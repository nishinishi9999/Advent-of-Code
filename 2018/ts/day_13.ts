...
class Cart extends Dirs {
  ...
  x                 :number;
  dir               :number;
  prev_intersection :number;
  has_colluded      :number;
  
  constructor(y :number, x :number, c :string) {
    super();
  ...
  move() {
    switch(this.dir) {
      case this.N: this.y--; break;
      case this.E: this.x++; break;
      case this.S: this.y++; break;
      case this.W: this.x--; break;
    }
  }
  ...
  turn(c :string) {
    switch(c) {
      case '/': {
        switch(this.dir) {
          case this.N: this.dir = this.E; break;
          case this.E: this.dir = this.N; break;
          case this.S: this.dir = this.W; break;
          case this.W: this.dir = this.S; break;
        }
        break;
      }
      case '\\': {
        switch(this.dir) {
          case this.N: this.dir = this.W; break;
          case this.E: this.dir = this.S; break;
          case this.S: this.dir = this.E; break;
          case this.W: this.dir = this.N; break;
        }
      }
    }
  }
  ...
  intersection() {
    ...
    switch(this.prev_intersection) {
      case 2: this.dir =  dirs[ ( dirs.indexOf(this.dir)+1) %  dirs.length ]; break;
      case 0: this.dir = _dirs[ (_dirs.indexOf(this.dir)+1) % _dirs.length ]; break;
    }
  }

  is_in(y :number, x :number) :boolean {
...
class Simulation extends Dirs {
  map             :string[][];
  carts           :Cart[];
  has_colluded    :boolean;
  collusion_point :number[];

  constructor(map :string[][], carts :Cart[]) {
    super();
  ...
  tick() {
    ...
    this.carts_left().forEach( cart => {
      if(!cart.has_colluded) {
        let y, x;
        switch(cart.dir) {
          case this.N: y = cart.y-1; x = cart.x;   break;
          case this.E: y = cart.y;   x = cart.x+1; break;
          case this.S: y = cart.y+1; x = cart.x;   break;
          case this.W: y = cart.y;   x = cart.x-1; break;
        }
...
function first(sim :Simulation) :string {
...
function second(sim :Simulation) :string {
  sim.print_map();
