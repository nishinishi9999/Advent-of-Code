...
function sort_timestamps(timestamps) {
    return timestamps.sort((t1, t2) => t1.month === t2.month && t1.day === t2.day && t1.hour === t2.hour ? t1.min - t2.min
...
function slept_the_most(timetable) {
    const id = {};
    Object.values(timetable).forEach(table => id[table[0].id] = (id[table[0].id] || 0) + table.filter(_ => _.sleep).length);
...
function second(timetable) {
    ...
    const max_mins = mins.map(min => Object.keys(min).length ? Object.values(min).sort((a, b) => b - a)[0] : 0);
