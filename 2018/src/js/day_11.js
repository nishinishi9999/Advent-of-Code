...
function create_map() {
    return Array(300).fill(0).map((_, y) => Array(300).fill(0).map((_, x) => new Cell(x, y)));
}
function set_power(map, serial_n) {
...
function first(map) {
    ...
    for (let y = 0; y < map.length - 2; y++) {
        for (let x = 0; x < map.length - 2; x++) {
            const power = map[y][x].power + map[y][x + 1].power + map[y][x + 2].power
