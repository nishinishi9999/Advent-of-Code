...
data Opcode = Cpy Arg Arg | Inc Arg | Dec Arg | Jnz Arg Arg | Tgl Arg | Out Arg
  deriving Show
...
out_update :: State -> Int -> State
out_update s n = s { _out = n:(_out s) }
...
exec :: State -> Bool
exec s
  | i s < 0 || i s >= V.length (op s) = False
  | not $ is_oscilating (_out s)      = False
  | length (_out s) >= 100            = True
  | otherwise = exec . inc_i $ case op s V.! i s of
    ...
    Tgl a   -> tgl s a
    Out a   -> out s a
  where is_oscilating xs = length xs < 2 || (take 2) xs == [0, 1] || (take 2) xs == [1, 0]

main :: IO ()
main = do
  input <- openFile "../input/day_25.txt" ReadMode >>= hGetContents
  let s = State {
      op   = format_input input
    , i    = 0
    , reg  = U.fromList [0, 0, 0, 0, 0]
    , _out = []
  }

  let first  = find (exec . reg_update s 0) [0..]
