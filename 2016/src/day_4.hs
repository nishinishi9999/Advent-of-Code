...
is_valid room = checksum room == make_checksum (name room)
  where make_checksum  = concatMap (take 1) . take 5 . sort_by_length . group . sort . filter (/= '-') 
...
main = do
  ...
  let second = (!!0) . filter (isInfixOf "north") . map (\room -> name room ++ " " ++ show (sector room)) $ decrypted
  print (first, second)
